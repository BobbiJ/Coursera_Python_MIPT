...
class GameSurface(ScreenHandle):
    ...
    def draw_hero(self):
        ...
        self.engine.hero.draw(self, min_x, min_y)
    ...
    def _get_top_left_corner(self):
        map_size = len(self.engine.map[0]), len(self.engine.map)
        hero_position = self.engine.hero.position

        hero_x = hero_position[0]
        hero_y = hero_position[1]

        map_x = map_size[0] - 2
        map_y = map_size[1] - 2

        screen_x = 8  # @TODO: calc this value
        screen_y = 5  # @TODO: calc this value

        min_x = hero_x - screen_x
        if min_x < 0:
            min_x = 0

        min_y = hero_y - screen_y
        if min_y < 0:
            min_y = 0
            ...
            for i in range(len(self.engine.map[0])):
                for j in range(len(self.engine.map)):
                    self.blit(
                        ...
                        ((i - min_x) * size, (j - min_y) * size)
                    )

        for obj in self.engine.objects:
            color = colors["red"] if isinstance(obj, Objects.Enemy) else colors["green"]
            pygame.draw.rect(self, color,
                             ((obj.position[0] - min_x) * size, (obj.position[1] - min_y) * size, 1 * size, 1 * size),
                             int(size / 2))

        pygame.draw.rect(self, colors["white"],
                         ((self.engine.hero.position[0] - min_x) * size, (self.engine.hero.position[1] - min_y) * size,
                          1 * size, 1 * size),
                         int(size / 2))
