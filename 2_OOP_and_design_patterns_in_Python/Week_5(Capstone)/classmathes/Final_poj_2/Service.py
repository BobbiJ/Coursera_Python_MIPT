...
def add_exp(engine, hero):
    engine.score += 0.1
    exp = random.choice([50, 100, 150])
    hero.exp += exp
    engine.notify(f"{exp} EXP added")
...
def apply_blessing(engine, hero):
    if hero.gold >= int(20 * 1.5**engine.level) - 2 * hero.stats["intelligence"]:
        ...
        if random.randint(0, 1) == 0:
            ...
            engine.hero = Objects.Fury(hero)
            engine.notify("Fury applied")
    else:
        engine.score -= 0.1
...
class SpecialMap(MapFactory):
    ...
    class Objects:
        ...
        def get_objects(self, _map):
            ...
            for obj_name, obj_count in self.config.items():
                prop = object_list_prob['enemies'][obj_name]
